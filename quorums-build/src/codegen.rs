...
impl prost_build::ServiceGenerator for GorumsGenerator {
    fn generate(&mut self, service: prost_build::Service, buf: &mut String) {
        ...
        for method in &service.methods {
            ...
            let req_bare = &method.input_type; // used inside register closures

            match call_type {
                CallType::RpcCall => {
                    client_fn_rpc(&mut client_fns, fn_name, &req_ty, &resp_ty, &full_path);
                ...
                CallType::Unicast => {
                    client_fn_unicast(&mut client_fns, fn_name, &req_ty, &full_path);
                    server_trait_fn_oneway(&mut server_trait_fns, fn_name, &req_ty);
                    register_handler(&mut register_calls, &full_path, fn_name, req_bare, false);
                }
                CallType::Multicast => {
                    client_fn_multicast(&mut client_fns, fn_name, &req_ty, &full_path);
                    server_trait_fn_oneway(&mut server_trait_fns, fn_name, &req_ty);
                    register_handler(&mut register_calls, &full_path, fn_name, req_bare, false);
                }
                CallType::QuorumCall => {
                    client_fn_qc(&mut client_fns, fn_name, &req_ty, &resp_ty, &full_path);
                    server_trait_fn_twoway(&mut server_trait_fns, fn_name, &req_ty, &resp_ty);
                    register_handler(&mut register_calls, &full_path, fn_name, req_bare, false);
                }
                CallType::OrderedQuorumCall => {
                    client_fn_oqc(&mut client_fns, fn_name, &req_ty, &resp_ty, &full_path);
                    server_trait_fn_twoway(&mut server_trait_fns, fn_name, &req_ty, &resp_ty);
                    register_handler(&mut register_calls, &full_path, fn_name, req_bare, false);
                }
                CallType::Correctable => {
                    client_fn_correctable(&mut client_fns, fn_name, &req_ty, &resp_ty, &full_path);
...
fn client_fn_rpc(out: &mut String, fn_name: &str, req_ty: &str, resp_ty: &str, path: &str) {
    let _ = writeln!(
        out,
        "    pub async fn {fn_name}(ctx: &::quorums::node::NodeContext, req: &{req_ty})"
    );
    // ::quorums::Error is the re-exported path (error module is private)
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(
        out,
        "        ::quorums::call_types::rpc_call(ctx, req, \"{path}\").await"
    );
    let _ = writeln!(out, "    }}");
}

fn client_fn_unicast(out: &mut String, fn_name: &str, req_ty: &str, path: &str) {
    let _ = writeln!(
        out,
        "    pub async fn {fn_name}(ctx: &::quorums::node::NodeContext, req: &{req_ty})"
    );
    ...
    let _ = writeln!(
        out,
        "        ::quorums::call_types::unicast(ctx, req, \"{path}\").await"
    );
    let _ = writeln!(out, "    }}");
}

fn client_fn_multicast(out: &mut String, fn_name: &str, req_ty: &str, path: &str) {
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(
        out,
        "        ::quorums::call_types::multicast(ctx, req, \"{path}\").await"
    );
    let _ = writeln!(out, "    }}");
}

fn client_fn_qc(out: &mut String, fn_name: &str, req_ty: &str, resp_ty: &str, path: &str) {
    let _ = writeln!(
        out,
        "    pub async fn {fn_name}(ctx: &::quorums::config::ConfigContext, req: &{req_ty})"
    );
    ...
    let _ = writeln!(
        out,
        "        ::quorums::call_types::quorum_call(ctx, req, \"{path}\").await"
    );
    let _ = writeln!(out, "    }}");
}

fn client_fn_oqc(out: &mut String, fn_name: &str, req_ty: &str, resp_ty: &str, path: &str) {
    let _ = writeln!(
        out,
        "    pub async fn {fn_name}(ctx: &::quorums::config::ConfigContext, req: &{req_ty})"
    );
    ...
    let _ = writeln!(
        out,
        "        ::quorums::call_types::ordered_quorum_call(ctx, req, \"{path}\").await"
    );
    let _ = writeln!(out, "    }}");
}

fn client_fn_correctable(out: &mut String, fn_name: &str, req_ty: &str, resp_ty: &str, path: &str) {
    let _ = writeln!(
        out,
        "    pub async fn {fn_name}(ctx: &::quorums::config::ConfigContext, req: &{req_ty})"
    );
    ...
    let _ = writeln!(
        out,
        "        ::quorums::call_types::correctable_call(ctx, req, \"{path}\").await"
    );
    let _ = writeln!(out, "    }}");
}
